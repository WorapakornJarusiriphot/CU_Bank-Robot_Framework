...
*** Test Cases ***
User registration with an account number shorter than 10 digits
...
User registration with an account number longer than 10 digits
    [Documentation]    EQ4B, EQ2, EQ3B
    Register User    1234567890000    1234    chaveehandsomeandcool    handsome
    Page Should Contain    Your account ID must be exactly 10 digits long

User registration with a non-digit account number
...
User registration with an existing account number
...
User registration with a non-digit password
...
User registration with a password shorter than 4 digits
...
User registration with a password longer than 4 digits
    [Documentation]    EQ1, EQ8B, EQ3A
    Register User    1234567890    12345678    Test    User
    Page Should Contain    Your password must be exactly 4 digits long.

User registration with a full name longer than 30 characters (including space)
